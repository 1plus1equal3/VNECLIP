...
# # Combine datasets
# combine_dataset = ConcatDataset([ktvic_dataset, uitvic_dataset])
# print(f"Combined dataset length: {len(combine_dataset)}")

# Initialize dataloader
BATCH_SIZE = 128
train_loader = DataLoader(ktvic_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=4, drop_last=True)
...
checkpoint_manager = CheckpointManager(save_dir='/root/Project/brick_vidgen/vnclip/ktvic_ckpt_160326', max_checkpoints=10)
start_epoch = 1  # Default start epoch if no checkpoint is found
try:
    ckpt_path = "/root/Project/brick_vidgen/vnclip/pretrained_ckpt_160326/checkpoint_epoch_145_2026-03-16.pth"
    start_epoch, _ = checkpoint_manager.load_checkpoint(vnclip, optimizer, ckpt_path)
    print(f"Resuming training from epoch {start_epoch}")
except IndexError:
    print("No checkpoint found, starting training from scratch.")

# Re-initialize optimizer with the model parameters (in case some layers were frozen/unfrozen)
start_epoch = 1
optimizer = optim.AdamW(vnclip.parameters(), lr=1e-5)

# Initialize Weights & Biases logger
wandb_logger = WandbLogger(project_name="PRETRAINED_VNECLIP", api_key=open("/root/Project/brick_vidgen/vnclip/wandb_key.txt").read())  # Replace with your actual W&B API key
