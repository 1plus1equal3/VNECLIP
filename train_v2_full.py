...
## KTVIC dataset
image_dir = "/root/Project/brick_vidgen/vnclip/dataset/ktvic_dataset/train-images"
annatation = load_json("/root/Project/brick_vidgen/vnclip/dataset/ktvic_dataset/train_data.json")

image_imgid2path = {}
caption_imgid2info = {}

# Build image_id2path and caption_id2info
for img_info in annatation["images"]:
    image_imgid2path[img_info["id"]] = os.path.join(image_dir, img_info["filename"])

for cap_info in annatation["annotations"]:
    if cap_info["image_id"] not in caption_imgid2info:
        caption_imgid2info[cap_info["image_id"]] = [{
            "caption_id": cap_info["id"],
            "caption": cap_info["caption"],
            "segment_caption": cap_info["segment_caption"]
        }]
    else:
        caption_imgid2info[cap_info["image_id"]].append({
            "caption_id": cap_info["id"],
            "caption": cap_info["caption"],
            "segment_caption": cap_info["segment_caption"]
        })

# Initialize ktvic dataset
ktvic_dataset = VNClipDataset(image_imgid2path, caption_imgid2info, tokenizer, transform=train_transform, use_wordseg=False)

## UITVIC dataset
image_dir = "/root/Project/brick_vidgen/vnclip/dataset/uitvic_dataset/coco_uitvic_train/coco_uitvic_train"
annatation = load_json("/root/Project/brick_vidgen/vnclip/dataset/uitvic_dataset/uitvic_captions_train2017.json")
...
# Build image_id2path and caption_id2info
for img_info in annatation["images"]:
    image_imgid2path[img_info["id"]] = os.path.join(image_dir, img_info["file_name"])
...
for cap_info in annatation["annotations"]:
    if cap_info["image_id"] not in caption_imgid2info:
        caption_imgid2info[cap_info["image_id"]] = [{
            "caption_id": cap_info["id"],
            "caption": cap_info["caption"],
            "segment_caption": word_tokenize(cap_info["caption"], format='text')
        }]
    else:
        caption_imgid2info[cap_info["image_id"]].append({
            "caption_id": cap_info["id"],
...
# Initialize dataset
uitvic_dataset = VNClipDataset(image_imgid2path, caption_imgid2info, tokenizer, transform=train_transform, use_wordseg=False)
...
BATCH_SIZE = 192
combined_train_dataset = ConcatDataset([train_dataset, ktvic_dataset, uitvic_dataset])
train_loader = DataLoader(combined_train_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
val_loader = DataLoader(val_dataset, batch_size=128, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
...
checkpoint_manager = CheckpointManager(save_dir='/root/Project/brick_vidgen/vnclip/checkpoints_v3_160326_full', max_checkpoints=10)
start_epoch = 1  # Default start epoch if no checkpoint is found
try:
    ckpt_path = "/root/Project/brick_vidgen/vnclip/checkpoints_v3_160326_phase2/checkpoint_epoch_39_2026-03-18.pth"
    start_epoch, _ = checkpoint_manager.load_checkpoint(vnclip, optimizer, ckpt_path)
    print(f"Resuming training from epoch {start_epoch}")
except IndexError:
    print("No checkpoint found, starting training from scratch.")

# Reset start_epoch to 1 for phase 2 training
start_epoch = 1
optimizer = optim.AdamW(vnclip.parameters(), lr=5e-5)

# Initialize Weights & Biases logger
wandb_logger = WandbLogger(project_name="VNECLIP_v2", api_key=open("/root/Project/brick_vidgen/vnclip/wandb_key.txt").read())  # Replace with your actual W&B API key
...
def train_step(batch):
    ...
    optimizer.zero_grad()
    loss = vnclip(images, encoded_inputs)
    loss.backward()
    optimizer.step()
    return loss.detach()
            ...
            if epoch % eval_epoch == 0:
                ...
                with torch.no_grad():
                    for batch in val_loader:
                        ...
                        update_metrics(eval_metrics, {"loss": loss})
